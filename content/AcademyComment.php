...
//taking page rating given by login user
        //$sql_comment="SELECT a.ID,a.PageId,a.Comment,date(a.CommentedON) AS CommentedON,u.NameReport,u.EmployeeNo,(SELECT COUNT(CommentLike) FROM LMCommentLike where CommentID= a.ID) as CommentLIKE FROM LMComment as a,userdetails as u where a.UserID= u.EmployeeNo and  a.PageId='". mysqli_real_escape_string($link,$_POST['PageID'])."' order by a.CommentedON desc";
        $sql_comment = "
    SELECT
        A.ID,
        A.PageId,
        ...
        U.NameReport,
        U.EmployeeNo,
        (SELECT GROUP_CONCAT(UserId) FROM LMCommentLike WHERE CommentID = A.ID) AS CmtLikedId,        
        (SELECT COUNT(CommentLike) FROM LMCommentLike WHERE CommentID = A.ID) AS CommentLIKE,
        (SELECT COUNT(LR.ID) FROM LMCommentReply AS LR WHERE LR.CommentID = A.ID GROUP BY CommentID) AS ReplyCount
    FROM
        LMComment AS A,
        userdetails AS U
    WHERE
    ...
    // END HERE ---Below code is used to disable like button and change text  
        ...
        <h6 class="comment-name">' . $data['NameReport'] . '</h6>
         ...
         <div class="comment-content" style="text-align:left;" id="comment' . $data["ID"] . '">' . $data['Comment'] . '</div>
            ...
            $html.='<a href="#" class="comment-like1" '.$disabled.' id=' . $data['ID'] . '>'.$content.'</a>';
    ...
    if ($_POST['fl'] == 'like') {
        ...
        // $getViewquery="SELECT count(pageId) as pgView FROM AcadmeyUsertracking WHERE pageId='". mysqli_real_escape_string($link,$_POST['PageID'])."'";
    ...
    if ($_POST['fl'] == 'insetCmt') {
        $insertQuery = "INSERT INTO LMComment (PageId,UserID,Comment,CommentedON) VALUES ('" . mysqli_real_escape_string($link, $_POST['PageID']) . "','" . mysqli_real_escape_string($link, $_POST['Id']) . "','" . mysqli_real_escape_string($link, $_POST['cmt']) . "',now())";
        mysqli_query($link, $insertQuery);
    }

    if ($_POST['fl'] == "pgLike") {
        $query = "SELECT * from LMPagelike where PageId ='" . mysqli_real_escape_string($link, $_POST['PageID']) . "' and UserID='" . mysqli_real_escape_string($link, $_POST['Id']) . "'";
        $result = mysqli_query($link, $query);

        if (mysqli_num_rows($result) > 0 ?
            :
            mysqli_query($link, "INSERT INTO LMPagelike (PageId,UserID,pageLike,likedON) VALUES ('" . mysqli_real_escape_string($link, $_POST['PageID']) . "','" . mysqli_real_escape_string($link, $_POST['Id']) . "','like',NOW()) "));
    }
// Close connection
    mysqli_close($link);
}
